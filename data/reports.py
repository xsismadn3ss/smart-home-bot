...
@query
def daily_average(conn, cursor: Cursor):
    sqlcommand = """
...
@query
def weekly_average(conn, cursor: Cursor):
    sqlcommnad = """INSERT INTO Weekly_Averages (avg_temperature, avg_humidity, week_start_date)
    SELECT AVG(T.value) AS avg_temperature, AVG(H.value) AS avg_humidity,
